...
public class UploadBgService extends IntentService {
    ...
    boolean isFollowUp = false,isService = false, isJobContinue = false, isAdhoc = false;
    String mFollowUpDate ="",mFollowUpPestType = "",mFollowUpServiceArea = "", mFollowUpJobNotes = "", mAdhocamt = "",
    mServiceFollowUpDate ="",mServiceFollowUpPestType = "",mServiceFollowUpServiceArea = "", mServiceFollowUpJobNotes = "",
    mctneFollowUpDate ="",mctneFollowUpPestType = "",mctneFollowUpServiceArea = "", mctneFollowUpJobNotes = "",
    madhocFollowUpDate ="",madhocFollowUpPestType = "",madhocFollowUpServiceArea = "", madhocFollowUpJobNotes = "";
                    ...
                    if (_appPrefs.getSERVICEID().contains("ADHOC_")) {
                        ...
                        try{
                            if(adhocRequest.get_id().length() >0){ // Passing _id Primary key used select model class
                                ...
                                Log.d("jhgsdjhsxghvvvv1/","xcv" + _appPrefs.getJobEndedTime().toString());
                                Log.d("jhgsdjhsxghvvvv1/","xcv" + Utils.getCurrentDateTime());
...
//                                adhocUploadRequest.setEndsat(Utils.getCurrentDateTime());
                                ...
                                Log.d("jhgsdjhsxghvvvv1","xcv" + arr_serviceRequest.get(0).getPestType());
                                ...
                                _appPrefs.saveJobEndedTime("" + Utils.getCurrentDateTime());

                                AdhocUploadResultParse(object);

                            }else{

                                Log.d("jhgsdjhsxghvvvv19","xcv" + _appPrefs.getJobEndedTime().toString());
                                Log.d("jhgsdjhsxghvvvv19","xcv" + Utils.getCurrentDateTime());
...
//                                adhocUploadRequest.setEndsat(Utils.getCurrentDateTime());
                                ...
                                Log.d("jhgsdjhsxghvvvv2","xcv" + arr_serviceRequest.get(0).getPestType());
                        ...
                        Log.d("/","xcv" + arr_serviceRequest.get(0).getPestType());
                        ...
                        // todo
                    //    uploadRequest.setScheduledate(Utils.getedDate(mList.get(0).getCreateDate()));
                        ...
                        if(ApiInterface.BASE_URL.trim().equals("https://mpest.isssg.biz:96/") || ApiInterface.BASE_URL.trim().equals("https://www.amanaasia.net:86/") ) {
                            uploadRequest.setScheduledate(Utils.getSingRequiredDate(mList.get(0).getCreateDate()));
                        }
                        else
                        {
                            uploadRequest.setScheduledate(Utils.getSingRequiredDate(mList.get(0).getCreateDate()));
...
//                        uploadRequest.setEndsat(Utils.getCurrentDateTime());
                        ...
                        uploadRequest.setFollowUp(isFollowUp);
                        uploadRequest.setFollowUpDate(mFollowUpDate);
                        uploadRequest.setFollowUpPestType(mFollowUpPestType);
                        uploadRequest.setFollowUpServiceArea(mFollowUpServiceArea);
                        uploadRequest.setFollowUpJobNotes(mFollowUpJobNotes);
                        uploadRequest.setServiceCall(isService);
                        uploadRequest.setContinueJob(isJobContinue);
                        uploadRequest.setAdhoc(isAdhoc);
                        uploadRequest.setAdhocAmount(mAdhocamt);
                        uploadRequest.setServiceCallDate(mServiceFollowUpDate);
                        uploadRequest.setServiceCallPestType(mServiceFollowUpPestType);
                        uploadRequest.setServiceCallServiceArea(mServiceFollowUpServiceArea);
                        uploadRequest.setServiceCallJobNotes(mServiceFollowUpJobNotes);
                        uploadRequest.setContinueJobDate(mctneFollowUpDate);
                        uploadRequest.setContinueJobPestType(mctneFollowUpPestType);
                        uploadRequest.setContinueJobServiceArea(mctneFollowUpServiceArea);
                        uploadRequest.setContinueJobJobNotes(mctneFollowUpJobNotes);
                        uploadRequest.setAdhocDate(madhocFollowUpDate);
                        uploadRequest.setAdhocPestType(madhocFollowUpPestType);
                        uploadRequest.setAdhocServiceArea(madhocFollowUpServiceArea);
                        uploadRequest.setAdhocUpJobNotes(madhocFollowUpJobNotes);
                        ...
                        Log.d("jhgsdjhsxghvvvv","xcv" + uploadRequest.toString());
                        ...
                        try {
                            ...
                            if (mStatusCode == 200) {
                                ...
                                if (message.equalsIgnoreCase("Job Already Completed")) {
                                    ...
                                    Call<Object> call2 = apiService.JobStatusUpdates(mJobId, "Completed", _appPrefs.getUserID());
    ...
    private void fetchAllDetails() {
        ...
        for (int i = 0; i < arr_servicedetail.size(); i++) {
            SelectedServices = SelectedServices + arr_servicedetail.get(i).getSS_remarks() + ",";
        }
        ...
        if (paymentCaptureRmModel != null) {
            ...
            isFollowUp = paymentCaptureRmModel.getFollowUp();
            mFollowUpDate = paymentCaptureRmModel.getFollowUpDate();
            mFollowUpPestType = paymentCaptureRmModel.getFollowUpPestType();
            mFollowUpServiceArea = paymentCaptureRmModel.getFollowUpServiceArea();
            mFollowUpJobNotes = paymentCaptureRmModel.getFollowUpJobNotes();
            isService = paymentCaptureRmModel.getServiceCall();
            isJobContinue = paymentCaptureRmModel.getContinueJob();
            isAdhoc = paymentCaptureRmModel.getAdhoc();
            mAdhocamt = paymentCaptureRmModel.getAmount();
            mServiceFollowUpDate = paymentCaptureRmModel.getServiceFollowUpDate();
            mServiceFollowUpPestType = paymentCaptureRmModel.getServiceFollowUpPestType();
            mServiceFollowUpServiceArea = paymentCaptureRmModel.getServiceFollowUpServiceArea();
            mServiceFollowUpJobNotes = paymentCaptureRmModel.getServiceFollowUpJobNotes();
            mctneFollowUpDate = paymentCaptureRmModel.getCtneFollowUpDate();
            mctneFollowUpPestType = paymentCaptureRmModel.getCtneFollowUpPestType();
            mctneFollowUpServiceArea = paymentCaptureRmModel.getCtneFollowUpServiceArea();
            mctneFollowUpJobNotes = paymentCaptureRmModel.getCtneFollowUpJobNotes();
            madhocFollowUpDate = paymentCaptureRmModel.getAdhocFollowUpDate();
            madhocFollowUpPestType = paymentCaptureRmModel.getAdhocFollowUpPestType();
            madhocFollowUpServiceArea = paymentCaptureRmModel.getAdhocFollowUpServiceArea();
            madhocFollowUpJobNotes = paymentCaptureRmModel.getAdhocFollowUpJobNotes();
        }
        ...
        if (arr_servicedetail.size() > 0) {
            ...
            for (int i = 0; i < arr_servicedetail.size(); i++) {
                ...
                if(arr_servicedetail.get(i).getSS_remarks().length() != 0 || arr_servicedetail.get(i).getOthers().length() != 0){

                    ServicesRequest servicesRequest = new ServicesRequest();
                    servicesRequest.setPestType(arr_servicedetail.get(i).getPestType());

                    if(arr_servicedetail.get(i).getOthers().length() >0 && arr_servicedetail.get(i).getSS_remarks().length() > 0){
                        servicesRequest.setServices(arr_servicedetail.get(i).getSS_remarks()+","+arr_servicedetail.get(i).getOthers());
                    }else if(arr_servicedetail.get(i).getOthers().length() ==0 && arr_servicedetail.get(i).getSS_remarks().length() > 0) {
                        servicesRequest.setServices(arr_servicedetail.get(i).getSS_remarks());
                    }else if(arr_servicedetail.get(i).getOthers().length() >0 && arr_servicedetail.get(i).getSS_remarks().length() == 0) {
                    ...
                    if(arr_servicedetail.get(i).getFM_remarks().length() >0)
                    {
                        servicesRequest.setFinding(arr_servicedetail.get(i).getFM_remarks());
                    }

                    if(arr_servicedetail.get(i).getRM_remarks().length() >0)
                    {
                        servicesRequest.setRecommendation(arr_servicedetail.get(i).getRM_remarks());
                    }

                    Log.d("uishdbjdgvuyxcgvjh1","cvbvb" + servicesRequest.getPestType());
                    arr_serviceRequest.add(servicesRequest);

                    Log.d("uishdbjdgvuyxcgvjh2","cvbvb" + arr_serviceRequest.get(0).getPestType());
                }
            }
        }

        Log.d("uishdbjdgvuyxcgvjh3","cvbvb" + arr_serviceRequest.get(0).getPestType());
        ...
        if (arr_previewMaterial.size() > 0) {
            ...
            for (int i = 0; i < arr_previewMaterial.size(); i++) {
                ...
                materialsRequest.setDilution(arr_previewMaterial.get(i).getDilution());

...
import com.amana.MpestISS.model.MaterialsCapturesModel;
...
public class MaterialFragment extends Fragment implements PestAdapter.ItemClickListener, MaterialQuantityAdapter.ItemClickListener,FragmentLifecycle {
    ...
    private void updatePestType() {
        ...
        for (int i = 0; arrayString.length > i; i++) {
            ...
            MasterDbLists.saveMatDetailsInDb(getActivity(), arrayString[i], "MATERIALS", _appPrefs.getSERVICEID().toString());
        }
    ...
    public void onItemClick(View view, int position, int ClickType) {
        ...
        switch (ClickType) {
            case AppConstants.TYPE_CLCIK_REMARKS:
                ...
                popup_QuantityUnit(position,materialsListAdapter.getItem(position).getMaterialName(),materialsListAdapter.getItem(position).getQuantity(),materialsListAdapter.getItem(position).getUnit(),materialsListAdapter.getItem(position).getDilution());
    ...
    private void popup_QuantityUnit(final int position,final String Materialname,final String Qty,final String Unit,final String Dilution) {
        ...
        final EditText edt_dilution = (EditText) promptsView.findViewById(R.id.input_dilution);
        ...
        edt_qty.setText(Qty);
        edt_dilution.setText(Dilution);
        spnr_unit.setText(Unit);
        txt_title.setText(Materialname);

            if(TextUtils.isEmpty(Qty) && TextUtils.isEmpty(Unit)){
                Materials materials = new Materials();
                materials = GetQtyUnitForMaterials(Materialname);

                if(!TextUtils.isEmpty(materials.getQuantity())){
                    edt_qty.setText(materials.getQuantity().toString());
                }

                if(!TextUtils.isEmpty(materials.getUnit())){
                    spnr_unit.setText(materials.getUnit().toString());
                }
            }
        ...
        btn_ok.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                alertDialog.dismiss();
                editedMaterialFieldSave(position, edt_qty.getText().toString(), spnr_unit.getText().toString(), edt_dilution.getText().toString());
            }
        });
        alertDialog.show();
    ...
    private void editedMaterialFieldSave(int position, String mQty, String mUnit, String mDilution) {
        ...
        for(MaterialsCapturesRmModel capturesRmModel: arr_materialsCapturesRmModels){
            ...
            materialsCapturesModel.setDilution(capturesRmModel.getDilution());
        ...
        materialsCapturesRmModels.get(position).setDilution(mDilution);

...
import com.amana.MpestISS.model.masterdetails.PaymentModeResponse;
import com.amana.MpestISS.model.masterdetails.ServiceTypeData;
import com.amana.MpestISS.model.masterdetails.ServiceTypeResponse;
import com.amana.MpestISS.model.masterdetails.SettingRemarksData;
import com.amana.MpestISS.model.masterdetails.SettingRemarksResponse;
import com.amana.MpestISS.model.masterdetails.TeamData;
import com.amana.MpestISS.model.masterdetails.TeamResponse;
...
import com.amana.MpestISS.model.realm.PaymentMode;
import com.amana.MpestISS.model.realm.RecommendationData;
import com.amana.MpestISS.model.realm.RecommendationResponse;
import com.amana.MpestISS.model.realm.ServiceType;
import com.amana.MpestISS.model.realm.SettingRemarks;
import com.amana.MpestISS.model.realm.Teams;
...
import com.amana.MpestISS.model.realm.FindingData;
import com.amana.MpestISS.model.realm.FindingResponse;
...
public class MasterDbLists {
    ...
    static ArrayList<FindingResponse> findingResponses = new ArrayList<>();
    static ArrayList<RecommendationResponse> recommendationResponses = new ArrayList<>();
                            ...
                            for (int i = 0; i < materialsResponse.size(); i++) {
                                ...
                                realm.executeTransaction(new Realm.Transaction() {
                                    ...
                                    public void execute(Realm realm) {
                                        ...
                                        materials.setQuantity(materialsResponse.get(finalI).getQuantity());
                                        materials.setUnit(materialsResponse.get(finalI).getUnit());
                                    }
                                });
                            }
                        }
     ...
     * gets TeamList
     */
    public static void getTeamList(final Context context, final String Client_id) {
    ...
    public static void getFinding(final Context context, final String Client_id) {
        Utils.showCustomDialog(context);
        final ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<FindingData> call = apiService.getFinding(Client_id);
        call.enqueue(new Callback<FindingData>() {
            @Override
            public void onResponse(Call<FindingData> call, Response<FindingData> response) {

                try {
                    if (response.body().getStatusCode() == 200) {
                        findingResponses = response.body().getData();

                        if (findingResponses.size() > 0) {
                            // File realmFile = new File(getFilesDir(), "default.realm");
                            final Realm realm = Realm.getDefaultInstance(); // opens db
                            realm.beginTransaction();
                            realm.where(FindingResponse.class).findAll().deleteAllFromRealm();
                            realm.commitTransaction();

                            for (int i = 0; i < findingResponses.size(); i++) {
                                ...
                                realm.executeTransaction(new Realm.Transaction() {
                                    ...
                                    public void execute(Realm realm) {
                                        ...
                                        //Create a user if there isn't one
                                        final FindingResponse serviceType = realm.createObject(FindingResponse.class); // Create a new String
                                        serviceType.setId(findingResponses.get(finalI).getId());
                                        serviceType.setPestType(findingResponses.get(finalI).getPestType());
                                        serviceType.setDescription(findingResponses.get(finalI).getDescription());
                                        serviceType.setIsActive(findingResponses.get(finalI).getIsActive());
                                        serviceType.setCreatedBy(findingResponses.get(finalI).getCreatedBy());
                                        serviceType.setUpdatedBy(findingResponses.get(finalI).getUpdatedBy());
                                        serviceType.setUpdatedDate(findingResponses.get(finalI).getUpdatedDate());
                                    }
                                });
                            }
                        }
                        ...
                        Utils.dismissDialog();
                        getRecommendations(context, Client_id);
                    }
                }
                catch (Exception e)
                {
                    Utils.dismissDialog();
                    e.printStackTrace();
                    AppLogger.verbose("FindingData", "Something Went wrong");
                }

            }

            @Override
            public void onFailure(Call<FindingData> call, Throwable t) {
                Utils.dismissDialog();
                AppLogger.verbose("FindingData", "unable to get details");
            }
        });
    }

    public static void getRecommendations(final Context context, final String Client_id) {
        Utils.showCustomDialog(context);
        final ApiInterface apiService = ApiClient.getClient().create(ApiInterface.class);
        Call<RecommendationData> call = apiService.getRecommendation(Client_id);
        call.enqueue(new Callback<RecommendationData>() {
            @Override
            public void onResponse(Call<RecommendationData> call, Response<RecommendationData> response) {

                try {
                    if (response.body().getStatusCode() == 200) {
                        recommendationResponses = response.body().getData();

                        if (recommendationResponses.size() > 0) {
                            // File realmFile = new File(getFilesDir(), "default.realm");
                            final Realm realm = Realm.getDefaultInstance(); // opens db
                            realm.beginTransaction();
                            realm.where(RecommendationResponse.class).findAll().deleteAllFromRealm();
                            realm.commitTransaction();

                            for (int i = 0; i < recommendationResponses.size(); i++) {
                                ...
                                realm.executeTransaction(new Realm.Transaction() {
                                    ...
                                    public void execute(Realm realm) {
                                        ...
                                        //Create a user if there isn't one
                                        final RecommendationResponse serviceType = realm.createObject(RecommendationResponse.class); // Create a new String
                                        serviceType.setId(recommendationResponses.get(finalI).getId());
                                        serviceType.setPestType(recommendationResponses.get(finalI).getPestType());
                                        serviceType.setDescription(recommendationResponses.get(finalI).getDescription());
                                        serviceType.setIsActive(recommendationResponses.get(finalI).getIsActive());
                                        serviceType.setCreatedBy(recommendationResponses.get(finalI).getCreatedBy());
                                        serviceType.setUpdatedBy(recommendationResponses.get(finalI).getUpdatedBy());
                                        serviceType.setUpdatedDate(recommendationResponses.get(finalI).getUpdatedDate());
                                    }
                                });
                            }
                        }
                ...
                catch (Exception e)
                {
                    Utils.dismissDialog();
                    e.printStackTrace();
                    AppLogger.verbose("RecomData", "Something Went wrong");
                }

            }

            @Override
            public void onFailure(Call<RecommendationData> call, Throwable t) {
                Utils.dismissDialog();
                AppLogger.verbose("RecomData", "unable to get details");
            }
        });
    }
     ...
     * @return
     */
    public static ArrayList<ListData1> getMytaskListFromdb(String mStatus, String mSearch) {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        RealmQuery<ListData1> query;
        if (mSearch.length() > 0) {
            //  mSearch = "?"+mSearch+"*";
            query = realm.where(ListData1.class).equalTo("Status", "" + mStatus)
                    .and()
        ...
        // Execute the query:
        RealmResults<ListData1> result1 = query.sort("Startsat").findAll();
        realm.commitTransaction();


        return (ArrayList<ListData1>) realm.copyFromRealm(result1);
    }
     ...
     * @return
     */
    public static ArrayList<ListData1> getMytaskListFromdb() {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        RealmQuery<ListData1> query;
        //  mSearch = "?"+mSearch+"*";
        query = realm.where(ListData1.class);

        // Execute the query:
        RealmResults<ListData1> result1 = query.findAll();
        realm.commitTransaction();
        return (ArrayList<ListData1>) realm.copyFromRealm(result1);
    }
    ...
    public static void UpdateMyTaskList(final int Count, final String mServiceID) {
        ...
        realm.executeTransaction(new Realm.Transaction() {
            @Override
            public void execute(Realm realm) {
                ListData1 obj = realm.where(ListData1.class).equalTo("ServiceID", mServiceID).findFirst();
                if (obj == null) {
                    obj = realm.createObject(ListData1.class);
                }
     ...
     * @return
     */
    public static ArrayList<ListData1> getServiceDetailsfromDb(String mServiceId) {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        RealmResults<ListData1> result1 = realm.where(ListData1.class).equalTo("ServiceID", mServiceId).findAll();
        realm.commitTransaction();
        return (ArrayList<ListData1>) realm.copyFromRealm(result1);
    }
    ...
    public static ArrayList<MultiSelectModel> getAllFindingList(String mPestType) {

        ArrayList<MultiSelectModel> multiSelectModels = new ArrayList<>();
        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();

        RealmQuery<FindingResponse> query;
        if (mPestType.length() > 0) {
            query = realm.where(FindingResponse.class).contains("PestType", mPestType).sort("Description");
        } else {
            query = realm.where(FindingResponse.class).sort("Description");
        }

        RealmResults<FindingResponse> result1 = query.findAll();
        realm.commitTransaction();

        ArrayList<FindingResponse> settingRemarks = (ArrayList<FindingResponse>) realm.copyFromRealm(result1);

        for (int i = 0; i < settingRemarks.size(); i++) {
            multiSelectModels.add(new MultiSelectModel(i, settingRemarks.get(i).getDescription().toString()));
        }
    ...
    public static ArrayList<MultiSelectModel> getAllRecommendationList(String mPestType) {

        ArrayList<MultiSelectModel> multiSelectModels = new ArrayList<>();
        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();

        RealmQuery<RecommendationResponse> query;
        if (mPestType.length() > 0) {
            query = realm.where(RecommendationResponse.class).contains("PestType", mPestType).sort("Description");
        } else {
            query = realm.where(RecommendationResponse.class).sort("Description");
        }

        RealmResults<RecommendationResponse> result1 = query.findAll();
        realm.commitTransaction();

        ArrayList<RecommendationResponse> settingRemarks = (ArrayList<RecommendationResponse>) realm.copyFromRealm(result1);
    ...
    public static ArrayList<MultiSelectModel> TeamMemberList(String mServiceid) {
        ...
        realm.beginTransaction();
        RealmQuery<ListData1> query1 = realm.where(ListData1.class).equalTo("ServiceID", mServiceid);
        // Execute the query:
        ListData1 datum = query1.findFirst();
        realm.commitTransaction();
        try {
     ...
     * @return
     */
    public static ListData1 GetTeamDetails(String mServiceid) {
    ...
    public static ServiceMaterialRMModel getServiceDetails(String mType, String mPestType, String mServiceID) {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        ServiceMaterialRMModel result1 = realm.where(ServiceMaterialRMModel.class)
                .equalTo("SS_Type", mType)
                .and()
                .equalTo("PestType", mPestType)
                .and()
                .equalTo("ServiceID", mServiceID)
    ...
    public static ServiceMaterialRMModel getfmDetails(String mFType,String mPestType, String mServiceID) {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        ServiceMaterialRMModel result1 = realm.where(ServiceMaterialRMModel.class)
                .equalTo("FM_Type", mFType)
                .and()
                .equalTo("PestType", mPestType)
                .and()
                .equalTo("ServiceID", mServiceID)
    ...
    public static ServiceMaterialRMModel getRmDetails(String mRType, String mPestType, String mServiceID) {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        ServiceMaterialRMModel result1 = realm.where(ServiceMaterialRMModel.class)
                .equalTo("RM_Type", mRType)
                .and()
                .equalTo("PestType", mPestType)
                .and()
                .equalTo("ServiceID", mServiceID)
    ...
    *
     * @param Materials
         * @return
     */
    public static Materials GetQtyUnitForMaterials(String Materials) {

        final Realm realm = Realm.getDefaultInstance(); // opens db
        realm.beginTransaction();
        Materials result1 = realm.where(Materials.class)
                .equalTo("materialsName", Materials)
                .findFirst();
        // Execute the query:
        realm.commitTransaction();
    ...
    public static void UpdateServiceRemarks(final String mServiceId, final String mType, final String mPest, final String mRemarks, final ArrayList<Integer> arrayList, final String mOthers) {
        final RealmList<Integer> arr_list = new RealmList<>();
        arr_list.addAll(arrayList);
        ...
        realm.executeTransaction(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                ...
                obj.setSS_remarks(mRemarks);
                obj.setServiceSelectedPositions(arr_list);
                obj.setOthers(mOthers);
            }
        });
    }

    public static void UpdateFindMaterialRemarks(final String mServiceId, final String mType, final String mPest, final String mRemarks, final ArrayList<Integer> findingList, final String mOthers) {
        final RealmList<Integer> arr_list = new RealmList<>();
        arr_list.addAll(findingList);
        ...
        realm.executeTransaction(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                ...
                obj.setFM_remarks(mRemarks);
                obj.setFindingPositions(arr_list);
                obj.setOthers(mOthers);
            }
        });
    }

    public static void UpdateRecommMaterialRemarks(final String mServiceId, final String mType, final String mPest, final String mRemarks, final ArrayList<Integer> recommenList, final String mOthers) {
        final RealmList<Integer> arr_list = new RealmList<>();
        arr_list.addAll(recommenList);
        ...
        realm.executeTransaction(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                ...
                obj.setRM_remarks(mRemarks);
                obj.setRecomPositions(arr_list);
                obj.setOthers(mOthers);
            }
        });
    }
    ...
    public static void UpdateServiceRemarks(final String mServiceId, final String mType, final String mPest, final String mOthers) {
    ...
    public static void UpdateFindMaterialRemarks(final String mServiceId, final String mType, final String mPest, final String mOthers) {
    ...
    public static void UpdateRecommMaterialRemarks(final String mServiceId, final String mType, final String mPest, final String mOthers) {
    ...
    public static void UpdateServiceRemarks(final String mServiceId, final String mType, final String mPest, final ArrayList<ServicesCapturesRmModel> servicesCapturesRmModel) {
     ...
     * @param mServiceID
     */
    public static void saveServiceDetailsInDb(Context context, final String mPest, final String mType, final String mfType, final String mrType, final String mServiceID) {
        ...
        if (serviceMaterialRMModels.size() == 0) {
            realm.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm realm) {
                    ...
                    serviceMaterialRMModel.setSS_Type(mType);
                    serviceMaterialRMModel.setFM_Type(mfType);
                    serviceMaterialRMModel.setRM_Type(mrType);
                    serviceMaterialRMModel.setPestType(mPest);
                    serviceMaterialRMModel.setSS_remarks("");
                    serviceMaterialRMModel.setFM_remarks("");
                    serviceMaterialRMModel.setRM_remarks("");
    ...
    public static void saveMatDetailsInDb(Context context, final String mPest, final String mType,final String mServiceID) {
...
/*    public static void saveImageDetails(final String BeforeAfter, final String mPest, final String ServiceID, final Bitmap bitmap){
     ...
     */
    public static void saveUpdatePaymentDetailsInDb(Context context, final String mServiceID, final String mPaymentMode, final String mTotalPayment, final String mPaymentNote, final String PaymentRemarks, final String mAreaPlanned, final String mAreaCompleted, final String mSOR, final Boolean isfollwup, final String follwupDate, final String pestType, final String serviceArea, final String jobNotes,final Boolean isService,final Boolean isJobContinue,final Boolean isAdhoc,final String amount,final String serviceDate, final String servicepestType, final String serArea, final String serviceNotes,final String ctneDate, final String ctnepestType, final String ctneserviceArea, final String ctnejobNotes,final String adhocDate, final String adhocpestType, final String adhocserviceArea, final String adhocjobNotes) {
        ...
        if (teamCaptureRmModels.size() == 0) {
            realm.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm realm) {
                    ...
                    paymentCaptureRmModel.setFollowUp(isfollwup);
                    paymentCaptureRmModel.setFollowUpDate(follwupDate);
                    paymentCaptureRmModel.setFollowUpPestType(pestType);
                    paymentCaptureRmModel.setFollowUpServiceArea(serviceArea);
                    paymentCaptureRmModel.setFollowUpJobNotes(jobNotes);
                    paymentCaptureRmModel.setServiceCall(isService);
                    paymentCaptureRmModel.setContinueJob(isJobContinue);
                    paymentCaptureRmModel.setAdhoc(isAdhoc);
                    paymentCaptureRmModel.setAmount(amount);
                    paymentCaptureRmModel.setServiceFollowUpDate(serviceDate);
                    paymentCaptureRmModel.setServiceFollowUpPestType(servicepestType);
                    paymentCaptureRmModel.setServiceFollowUpServiceArea(serArea);
                    paymentCaptureRmModel.setServiceFollowUpJobNotes(serviceNotes);
                    paymentCaptureRmModel.setCtneFollowUpDate(ctneDate);
                    paymentCaptureRmModel.setCtneFollowUpPestType(ctnepestType);
                    paymentCaptureRmModel.setCtneFollowUpServiceArea(ctneserviceArea);
                    paymentCaptureRmModel.setCtneFollowUpJobNotes(ctnejobNotes);
                    paymentCaptureRmModel.setAdhocFollowUpDate(adhocDate);
                    paymentCaptureRmModel.setAdhocFollowUpPestType(adhocpestType);
                    paymentCaptureRmModel.setAdhocFollowUpServiceArea(adhocserviceArea);
                    paymentCaptureRmModel.setAdhocFollowUpJobNotes(adhocjobNotes);
            ...
            realm.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm realm) {
                    ...
                    obj.setSOR(mSOR);
                    obj.setFollowUp(isfollwup);
                    obj.setFollowUpDate(follwupDate);
                    obj.setFollowUpPestType(pestType);
                    obj.setFollowUpServiceArea(serviceArea);
                    obj.setFollowUpJobNotes(jobNotes);
                    obj.setServiceCall(isService);
                    obj.setContinueJob(isJobContinue);
                    obj.setAdhoc(isAdhoc);
                    obj.setAmount(amount);
                    obj.setServiceFollowUpDate(serviceDate);
                    obj.setServiceFollowUpPestType(servicepestType);
                    obj.setServiceFollowUpServiceArea(serArea);
                    obj.setServiceFollowUpJobNotes(serviceNotes);
                    obj.setCtneFollowUpDate(ctneDate);
                    obj.setCtneFollowUpPestType(ctnepestType);
                    obj.setCtneFollowUpServiceArea(ctneserviceArea);
                    obj.setCtneFollowUpJobNotes(ctnejobNotes);
                    obj.setAdhocFollowUpDate(adhocDate);
                    obj.setAdhocFollowUpPestType(adhocpestType);
                    obj.setAdhocFollowUpServiceArea(adhocserviceArea);
                    obj.setAdhocFollowUpJobNotes(adhocjobNotes);
    ...
    public static ArrayList<ServiceMaterialRMModel> getServiceFromDB(String mServiceID, String mType) {
    ...
    public static void UploadJobStatus(final String mServiceID, final String mStatus) {
        ...
        realm.executeTransaction(new Realm.Transaction() {
            @Override
            public void execute(Realm realm) {
                ListData1 obj = realm.where(ListData1.class)
                        .equalTo("ServiceID", mServiceID)
                        .findFirst();
                if (obj == null) {
                    obj = realm.createObject(ListData1.class);
    ...
    public static void UploadEndTime(final String mServiceID, final String mEndTime) {
        ...
        realm.executeTransaction(new Realm.Transaction() {
            ...
            public void execute(Realm realm) {
                ...
                obj.getSchedulerTrackingdetails().get(0).setEndTime(mEndTime);
            }
        });
    }
    ...
    public static int checkInProgressExist() {
        ...
        realm.beginTransaction();
        RealmResults<ListData1> result1 = realm.where(ListData1.class)
                .equalTo("Status", "In-Progress")
                .findAll();
        // Execute the query:
        realm.commitTransaction();

        ArrayList<ListData1> teamCaptureRmModels = (ArrayList<ListData1>) realm.copyFromRealm(result1);
    ...
    public static String getCustomerName_InProgressExist() {
        ...
        realm.beginTransaction();
        ListData1 result1 = realm.where(ListData1.class)
                .equalTo("Status", "In-Progress")
                .findFirst();
        // Execute the query:
        realm.commitTransaction();
        ...
        if (Paymentresult != null) {
            ...
            realm.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm realm) {
                    ...
                    logPaymentCaptureRmModel.setFollowUp(Paymentresult.getFollowUp());
                    logPaymentCaptureRmModel.setFollowUpDate(Paymentresult.getFollowUpDate());
                    logPaymentCaptureRmModel.setFollowUpPestType(Paymentresult.getFollowUpPestType());
                    logPaymentCaptureRmModel.setFollowUpServiceArea(Paymentresult.getFollowUpServiceArea());
                    logPaymentCaptureRmModel.setFollowUpJobNotes(Paymentresult.getFollowUpJobNotes());
                    logPaymentCaptureRmModel.setServiceCall(Paymentresult.getServiceCall());
                    logPaymentCaptureRmModel.setContinueJob(Paymentresult.getContinueJob());
                    logPaymentCaptureRmModel.setAdhoc(Paymentresult.getAdhoc());
                    logPaymentCaptureRmModel.setAmount(Paymentresult.getAmount());
                    logPaymentCaptureRmModel.setServiceFollowUpDate(Paymentresult.getServiceFollowUpDate());
                    logPaymentCaptureRmModel.setServiceFollowUpPestType(Paymentresult.getServiceFollowUpPestType());
                    logPaymentCaptureRmModel.setServiceFollowUpServiceArea(Paymentresult.getServiceFollowUpServiceArea());
                    logPaymentCaptureRmModel.setServiceFollowUpJobNotes(Paymentresult.getServiceFollowUpJobNotes());
                    logPaymentCaptureRmModel.setCtneFollowUpDate(Paymentresult.getCtneFollowUpDate());
                    logPaymentCaptureRmModel.setCtneFollowUpPestType(Paymentresult.getCtneFollowUpPestType());
                    logPaymentCaptureRmModel.setCtneFollowUpServiceArea(Paymentresult.getCtneFollowUpServiceArea());
                    logPaymentCaptureRmModel.setCtneFollowUpJobNotes(Paymentresult.getCtneFollowUpJobNotes());
                    logPaymentCaptureRmModel.setAdhocFollowUpDate(Paymentresult.getAdhocFollowUpDate());
                    logPaymentCaptureRmModel.setAdhocFollowUpPestType(Paymentresult.getAdhocFollowUpPestType());
                    logPaymentCaptureRmModel.setAdhocFollowUpServiceArea(Paymentresult.getAdhocFollowUpServiceArea());
                    logPaymentCaptureRmModel.setAdhocFollowUpJobNotes(Paymentresult.getAdhocFollowUpJobNotes());
